...
```
  ...
  │  training verdict) or manual request      │
  └──────────┬────────────────────────────────┘
             │
  Step 1     ▼  READ TOKEN
  ┌───────────────────────────────────────────┐
  ...
  ┌───────────────────────────────────────────┐
  │  Quick summary: devices, training status, │
  │  race countdown, recovery, device warnings│
  └─────────────────────────────────────────  ┘
```
...
- **Automatically** via HEARTBEAT.md morning cron (runs before the training verdict skill)
...
- **Token missing** → Print and stop:
  ...
  Run: claw workspace install betterness/marathon
  ```
...
| `getUserHealthData` | Latest sleep, HRV, activity, training load |
...
- `memory/health-snapshot.md` — Full replacement with latest training + health data

Merged (not replaced):
- `memory/preferences.md` — Add newly connected devices to device list. Keep existing user-set preferences (race date, goal pace, training plan, long run day). Only update auto-inferred values if the user hasn't manually overridden them.
...
📊 Since you installed ({days} days ago):
   ...
   Weekly mileage: {old} → {new} mi ({delta})
...
```
┌─────────────────────────────────────────────────┐
│  🔄 MARATHON DATA REFRESH                       │
│                                                 │
│  {Name} · Bio Age {score} · {device_count} dev  │
│                                                 │
│  DEVICES        LAST SYNC    STATUS              │
│  Garmin FR 265  2h ago       ✅ Active           │
│  WHOOP 4.0      4h ago       ✅ Active           │
│  Oura Ring      14d ago      ⚠️  Stale           │
│                                                 │
│  TRAINING STATUS                                │
│  Weekly: 42mi (target: 50mi) · Phase: Build 2   │
│  Long run: 18mi (Sat) · Load: moderate          │
│  Race: Boston Marathon in 47 days               │
│                                                 │
│  HRV 7d: ▃▅▇▅▄▆█ 62ms (↑0.8)                   │
│  Recovery: 🟢 Good — ready for tempo today       │
└─────────────────────────────────────────────────┘
```
...
```
⚠️ Oura Ring hasn't synced in 14 days.
...
- If everything is normal: update files, no output (the training verdict skill handles the morning message)
- If device warnings: append warnings to the training verdict output
...
2. **Merge preferences, don't replace.** New devices get added, but race date, goal pace, and training plan choices are sacred.
