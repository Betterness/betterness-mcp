...
```
  ...
  │  ride readiness) or manual request        │
  └──────────┬────────────────────────────────┘
             │
  Step 1     ▼  READ TOKEN
  ┌───────────────────────────────────────────┐
  ...
  ┌───────────────────────────────────────────┐
  │  Quick summary: devices, power profile,   │
  │  ride readiness, device warnings          │
  └─────────────────────────────────────────  ┘
```
...
- **Automatically** via HEARTBEAT.md morning cron (runs before the ride readiness skill)
...
- **Token missing** → Print and stop:
  ...
  Run: claw workspace install betterness/road-cycling
  ```
...
| `getUserHealthData` | Latest sleep, HRV, activity, power data, training load |
...
- `memory/health-snapshot.md` — Full replacement with latest power + health data

Merged (not replaced):
- `memory/preferences.md` — Add newly connected devices to device list. Keep existing user-set preferences (FTP override, zone preferences, W/kg target, ride days). Only update auto-inferred values if the user hasn't manually overridden them.
...
```
┌─────────────────────────────────────────────────┐
│  🔄 CYCLING DATA REFRESH                        │
│                                                 │
│  {Name} · FTP {ftp}W ({w_kg} W/kg) · {dev} dev  │
│                                                 │
...
│                                                 │
│  CTL: 72 · ATL: 58 · TSB: +14 (fresh)           │
│  Weekly TSS: 480/550                             │
│  FLAGGED: Ferritin ⚠️ Low                        │
│  HRV 7d: ▃▅▇▅▄▆█ 62ms (↑0.8)                   │
│  Ride readiness: 🟢 Go for watts                 │
└─────────────────────────────────────────────────┘
```
...
- If everything is normal: update files, no output (the ride readiness skill handles the morning message)
- If device warnings: append warnings to the ride readiness output
...
2. **Merge preferences, don't replace.** New devices get added, but FTP overrides, zone preferences, and ride days are sacred.
