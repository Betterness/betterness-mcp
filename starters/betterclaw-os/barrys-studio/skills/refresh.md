...
```
  ...
  │  class readiness) or manual request       │
  └──────────┬────────────────────────────────┘
             │
  Step 1     ▼  READ TOKEN
  ┌───────────────────────────────────────────┐
  ...
  ┌───────────────────────────────────────────┐
  │  Quick summary: devices, class activity,  │
  │  intensity balance, device warnings       │
  └─────────────────────────────────────────  ┘
```
...
- **Automatically** via HEARTBEAT.md morning cron (runs before the class readiness skill)
...
- **Token missing** → Print and stop:
  ...
  Run: claw workspace install betterness/barrys-studio
  ```
...
| `getUserHealthData` | Latest sleep, HRV, activity, strain, recovery |
...
- `memory/health-snapshot.md` — Full replacement with latest class + recovery data

Merged (not replaced):
- `memory/preferences.md` — Add newly connected devices to device list. Keep existing user-set preferences (studio, class types, weekly target, rest days). Only update auto-inferred values if the user hasn't manually overridden them.
...
📊 Since you installed ({days} days ago):
   ...
   Weekly classes: {old} → {new} ({delta})
   Avg strain: {old} → {new} ({delta})
...
│                                                 │
│  This week: 3/4 classes · Strain: 38.2           │
│  Balance: 2 hard / 1 moderate                    │
│  HRV 7d: ▃▅▆▅▃▅▇ 58ms (↑0.6)                   │
│  Class call: 🟢 Go hard — fully recovered        │
└─────────────────────────────────────────────────┘
```
...
- If everything is normal: update files, no output (the class readiness skill handles the morning message)
- If device warnings: append warnings to the class readiness output
...
2. **Merge preferences, don't replace.** New devices get added, but studio choice, class types, and weekly targets are sacred.
